...
function sendPasswordResetEmail(email, resetURL) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (!user)
            throw new errorService_1.AppError("There is no user with email address", 404);
        const resetToken = user.createPasswordResetToken();
        yield user.save({ validateBeforeSave: false });
        const message = `Forgot your password? Submit a PATCH request with your new password and passwordConfirm to: ${resetURL + resetToken}.\nIf you didn't forget your password, please ignore this email!`;
        try {
            yield (0, utilService_1.sendEmail)({
                email: user.email,
            ...
            yield user.save({ validateBeforeSave: false });
            throw new errorService_1.AppError("There was an error sending the email. Try again later!", 500);
        }
    });
}
